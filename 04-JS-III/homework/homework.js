...
function dePalabrasAFrase(palabras) {
  ...
  for (let i = 0; i < palabras.length - 1; i++) {
    stringCompleto += palabras[i] + " ";
  }
  stringCompleto += palabras[palabras.length - 1];
...
function breakStatement(numero) {
  ...
  var listaDosEnDos = []; // -4
  for (let i = 1; i <= 10; i++) {
    listaDosEnDos.push((numero += 2));
    if (i === listaDosEnDos[i - 1]) {
      break;
    }
  }
  if (listaDosEnDos.length === 10) {
    return listaDosEnDos;
  }
